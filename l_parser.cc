...
namespace
{
	...
	public:
		...
		std::vector<std::pair<double, std::string>> readRulePairs()
		{
			...
			if (c == '"') 
			{
				pairs.push_back(std::pair<double, std::string>(1, readQuotedString()));
			}
			else if (c == '{')
			{
				getChar();
				...
				while (c != '}')
				{
					...
					assertChars("(");
					std::string rule = readQuotedString();
					skip_comments_and_whitespace();
					assertChars(",");
					skip_comments_and_whitespace();
					double chance = readDouble();
					skip_comments_and_whitespace();
					assertChars(")");
					skip_comments_and_whitespace();
					pairs.push_back(std::pair<double, std::string>(chance, rule));
					c = (char)getChar();
				}
			}
			else throw LParser::ParserException("Did not find a valid rule", line, col);
	...
	void parse_rules(std::set<char> const& alphabet, std::map<char, std::vector<std::pair<double, std::string>>>& rules, stream_parser& parser, bool parse2D)
	{
		...
		while (true)
		{
			...
			std::vector<std::pair<double, std::string>> rule = parser.readRulePairs();
			double total = 0;

			for (std::vector<std::pair<double, std::string>>::iterator it = rule.begin(); it != rule.end(); ++it)
			{
				if (!isValidRule(alphabet, it->second, parse2D))
			...
			if (round(total * 1000.0) / 1000.0 != 1)
				throw LParser::ParserException("Total chance of combined rules is not 1 in rule specification " + std::to_string(total), parser.getLine(), parser.getCol());
...
LParser::LSystem::LSystem() : alphabet(), drawfunction(), initiator(""), angle(0.0), replacementrules(), nrIterations(0)
{}

LParser::LSystem::LSystem(LSystem const&system) : alphabet(system.alphabet), drawfunction(system.drawfunction), initiator(system.initiator), angle(system.angle), replacementrules(system.replacementrules), nrIterations(system.nrIterations)
{}
...
std::string LParser::LSystem::get_replacement(char c) const
{
	...
	std::vector<std::pair<double, std::string>> rulepairs = replacementrules.find(c)->second;

	for (std::vector<std::pair<double, std::string>>::iterator it = rulepairs.begin(); it != rulepairs.end(); ++it)
	{
		chance += it->first;
		if (random <= chance)
		{
